...
@click.option("--aws-credentials-csv-file", "-c", default=os.getenv("AWS_K8S_CREDENTIALS"), help="the aws credentials csv file")
...
def deploy_helm(chart_name, gcp_project, gcp_credentials, tag, deploy_env, gcp_cluster, aws_credentials_csv_file, aws_region, aws_cluster, deploy_type):
    ...
    try:
        ...
        if deploy_type in ["AWS", "MULTI"]:
           install_helm_on_aws(aws_credentials_csv_file, aws_region, aws_cluster, chart_name, chart_file)
